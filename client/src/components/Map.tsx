...
export function MapView({
  ...
  const init = usePersistFn(async () => {
    try {
      ...
      console.error(error);
      setMapError("Kaart kan niet worden geladen. Controleer de API-configuratie.");
      return;
    }

    if (!window.google?.maps) {
      setMapError("Kaartbibliotheek is niet beschikbaar.");
      return;
    }
  ...
  if (mapError) {
    return (
      <div
        className={cn(
          "w-full h-[500px] rounded-md border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800",
          className
        )}
      >
        {mapError}
